...
针对纵向裂缝、横向裂缝和网状裂缝细长、低对比度、目标尺寸小的问题，项目将性能增强集中在两步：

1. **裂缝标注清理**：复核漏标、错类、框过松和框过紧样本，统一坐标边界与类别定义；把确认无法可靠判断的样本放入隔离清单，不直接当作背景参与训练。
2. **切片检测**：训练和推理阶段将高分辨率道路图切成带重叠区域的局部窗口，检测后把局部坐标映射回原图并做跨切片 NMS，减少细裂缝在缩放过程中的信息损失。

两步需要在同一数据划分、同一独立留出集上对照验证，至少同时报告整体及各类别的 mAP50、mAP50–95、召回率和端到端延迟。切片通常会增加小目标召回，但也会增加推理耗时和重复框处理成本；具体提升以重新准备数据并完成实验后的报告为准。
